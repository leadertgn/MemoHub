...
def get_current_user_optional(
    credentials: HTTPAuthorizationCredentials = Depends(bearer_scheme_opt),
    session: Session = Depends(get_session)
):
    """
    Similaire à get_current_user, mais ne lève pas d'erreur si l'utilisateur
    n'est pas connecté. Retourne l'utilisateur ou None.
    """
    if not credentials:
        return None
